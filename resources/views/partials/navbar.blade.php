        ...
        <div class="hidden md:flex items-center space-x-8 text-sm font-medium tracking-wide">
            ...
            @auth
                <div class="relative" @click.away="openAulas = false">
                    ...
                    <button @click="openAulas = !openAulas" class="flex items-center gap-2 hover:text-[#c0ff01] transition">
        ...
        <div class="hidden md:flex items-center gap-3">
            @auth
                ...
                <form action="{{ route('filament.admin.auth.logout') }}" method="POST">
                    @csrf
                    <button type="submit" class="text-sm px-4 py-2 rounded-md hover:bg-white/5 transition text-white/80">
                        Sair
                    </button>
                </form>
            @else
